...
#
# Tests: mcp_server with some-mcp (stdio transport)
#
...
swarm = SwarmSDK.build do
  ...
  agent(:mcp_agent) do
    model("gpt-5-nano")
    provider("openai")
    system_prompt("You have access to MCP tools from some-mcp. List available tools when asked.")
    ...
    mcp_server(
      ...
      args: ["example-mcp-bridge"],
      env: {
        MCP_API_TOKEN: ENV["VAULT_TOKEN"],
        MCP_TARGET_URL: "https://vault.example.com/mcp",
      },
    )
  end
end
...
puts "(Agent should have access to some-mcp tools)"
puts ""
